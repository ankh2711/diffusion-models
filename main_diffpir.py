...
def parse_args():
    ...
    parser.add_argument("--output", type=str, default="result/diffpir/restored.png")
...
def main():
    ...
    model_name = os.path.basename(model_path)  # 256x256_diffusion_uncond, diffusion_ffhq_10m; set diffusino model
    ...
    mask_path = args.mask
    # mask_name = 'gt_keep_masks/19-0.png'  # mask path for loading mask img
    load_mask = True
    ...
    generate_mode = 'DiffPIR2'  # repaint; vanilla; DiffPIR2
    ...
    results = os.path.join(cwd, 'results2')  # fixed
    ...
    ) if model_name == 'diffusion_ffhq_10m.pt' \
        else dict(
        model_path=model_path,
        num_channels=256,
        num_res_blocks=2,
        ...
        for idx, img in enumerate(L_paths):
            ...
            for i in range(len(seq)):
                ...
                for u in range(iter_num_U):
                    ...
                    # analytic solution
                    if (generate_mode == 'DiffPIR2') and not (seq[i] == seq[-1]):
                        # solve sub-problem
                        if sub_1_analytic:
                            if model_out_type == 'pred_xstart':
            ...
            if generate_mode in ['repaint', 'DiffPIR2']:
            ...
            if calc_LPIPS:
                ...
                logger.info(
                    '{:->4d}--> {:>10s} PSNR: {:.4f}dB LPIPS: {:.4f} ave LPIPS: {:.4f}'.format(idx, img_name + ext,
            ...
            if torch.cuda.is_available():
                ...
                gpu_mem = 0
            logger.info(f'{img_name}{ext} | Time: {elapsed_time:.2f}s | GPU memory: {gpu_mem:.2f} MB')
            ...
            if save_progressive:
                ...
                util.imsave(img_total * 255., os.path.join(E_path,
                                                           img_name + '_process_lambda_{:.3f}_{}{}'.format(lambda_,
                                                                                                           current_time,
                                                                                                           ext)))
                images = []
                ...
                if generate_mode in ['repaint', 'DiffPIR2']:
                    ...
                    if save_progressive_mask:
                        util.imsave(img_total * 255., os.path.join(E_path,
                                                                   img_name + '_process_mask_lambda_{:.3f}_{}{}'.format(
                                                                       lambda_, current_time, ext)))
