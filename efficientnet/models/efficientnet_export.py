...
class EfficientNet(nn.Module):
    ...
    def __init__(self, width_mult=1.0, depth_mult=1.0, dropout_rate=0.2, num_classes=1000, isAggregationClass=False, noiseClassIds = [0]):
        ...
        # yapf: enable
        self._noiseClass = torch.tensor(noiseClassIds, dtype=torch.int32)
        self._notNoiseClass = torch.tensor([ i for i in range(num_classes) if i not in noiseClassIds], dtype=torch.int32)
        self._noiseClassNeedSum = self._noiseClass.size(0) > 1
        self._notNoiseClassNeedSum = self._notNoiseClass.size(0) > 1

        self._numClasses = num_classes
        self._isAggregationClass = isAggregationClass
    ...
    def _AggregationResultTensor(self, result, tensorIds, needSum):
        aggregationResult = torch.index_select(result,-1, tensorIds)
        if(needSum):
            aggregationResult = aggregationResult.sum(-1).view(-1,1)
    ...
    def forward(self, x):
        ...
        if(self._isAggregationClass):
            noiseClassResult = self._AggregationResultTensor(result, self._noiseClass, self._noiseClassNeedSum)
            notNoiseClassResult = self._AggregationResultTensor(result, self._notNoiseClass, self._noiseClassNeedSum)
            result = torch.cat((noiseClassResult, notNoiseClassResult), dim=1)
        return result
